document.addEventListener("DOMContentLoaded", async function () {
    ...
    async function fetchIcons(selectedIcon) {
        try {
            const response = await fetch(chrome.runtime.getURL("assets/arrow_styles/icon-styles.json"));
    ...
    function displayIcons(selectedIcon) {
        iconGrid.innerHTML = "";
        const iconsToShow = expanded ? allIcons : allIcons.slice(0, 4);

        iconsToShow.forEach(icon => {
            const img = document.createElement("img");
            img.src = chrome.runtime.getURL(`assets/arrow_styles/${icon}`);
